...
import {
  exportProductionOutput,
  exportWorkshopPeriod,
  getProductionOutputDetails,
  getProductionOutputSummary,
  getWorkshopPeriodDetails,
  getWorkshopPeriodSummary,
} from '@/api/report'

defineOptions({ name: 'ProductionStyleReportView' })

/**
 * 产量报表与车间时段报表共用视图：查询条件、汇总卡片、明细表格结构一致，
 * 仅后端接口与文案不同，通过 kind 切换。
 */
const props = defineProps<{
  title: string
  description: string
  kind: 'production_outputs' | 'workshop_periods'
}>()

const reportApi =
  props.kind === 'production_outputs'
    ? {
        summary: getProductionOutputSummary,
        details: getProductionOutputDetails,
        exportFile: exportProductionOutput,
      }
    : {
...
const {
  ...
  handleReset,
  handleExport,
} = useProductionStyleReport(reportApi)
