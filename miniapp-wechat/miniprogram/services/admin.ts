...
function canAssignAdminRole(): boolean {
  const currentUser = (wx.getStorageSync('mes_user') || {}) as { roleCodes?: string[] }
  return (currentUser.roleCodes || []).includes('ADMIN')
}
...
export function getAssignableRoles(): Promise<SystemRole[]> {
  const roles = isMockMode() ? mockAssignableRoles() : request<SystemRole[]>({ url: '/api/system/roles', method: 'GET' })
  return roles.then(items => canAssignAdminRole() ? items : items.filter(item => item.roleCode !== 'ADMIN'))
}
