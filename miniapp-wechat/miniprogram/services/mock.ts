...
export async function mockAssignableRoles(): Promise<SystemRole[]> { return wait(MOCK_ROLES) }
...
export async function mockUpdateUserAssignment(id: number, data: UserAssignmentRequest): Promise<void> { const users = readMockUsers(); const user = users.find(item => item.id === id); if (!user) throw new Error('用户不存在'); const currentUser = (wx.getStorageSync('mes_user') || {}) as MiniAppLoginResponse; const canAssignAdmin = (currentUser.roleCodes || []).includes('ADMIN'); const requestedRoles = MOCK_ROLES.filter(item => data.roleIds.includes(item.id)); if (!canAssignAdmin && requestedRoles.some(item => item.roleCode === 'ADMIN')) throw new Error('只有管理员可以分配管理员职位'); const preservedAdmin = !canAssignAdmin && user.roleCodes.includes('ADMIN') ? MOCK_ROLES[0] : undefined; const roles = [...(preservedAdmin ? [preservedAdmin] : []), ...requestedRoles.filter(item => !preservedAdmin || item.roleCode !== 'ADMIN')]; if (!roles.length) throw new Error('请至少为用户保留一个职位'); user.roleIds = roles.map(item => item.id); user.roleCodes = roles.map(item => item.roleCode); user.roleNames = roles.map(item => item.roleName); user.workshopId = data.workshopId; user.lineId = data.lineId; saveMockUsers(users); await wait(undefined) }
