...
| B | `barcode`、`scene` | 条码和现场执行；`report` 当前尚无生产 Java 包 |
| C | `equipment`、`device`、`quality`、`andon` | 设备、设备接入、质量和安灯 |

本轮只新增测试源文件和测试文档，不修改生产代码、数据库迁移、构建配置或既有测试。
用户已明确 B 组测试允许在 `fanjiaquan` 分支开发；后续如提交或推送，只针对该分支。
...
- B：条码常量/日期/变量/流水组合、生产参数默认值、任务/工序/批次状态；
...
本轮新增的 15 个文件是按维度组织的补充套件，不替换既有功能测试。当前 `report` 生产包尚未
落地，因此没有对不存在的实现伪造测试；待 `report` 功能实现后，应按同样四个维度新增独立文件。
...
结果：`BUILD SUCCESSFUL`，15 个新增测试类、60 个新增用例全部通过，0 失败、0 错误、0 跳过。
...
执行结果：`BUILD SUCCESSFUL`；105 个测试类、645 个用例，0 失败、0 错误、0 跳过。该命令应在提交前执行，确认新增维度套件没有影响既有模块测试。若某次结果包含集成环境
