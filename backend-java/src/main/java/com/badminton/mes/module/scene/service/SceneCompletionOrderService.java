...
public class SceneCompletionOrderService {
    ...
    public SceneCompletionOrderService(CompletionOrderRepository completionOrderRepository,
                                       ...
                                       SceneProductionTaskRepository productionTaskRepository,
    ...
    public Long createCompletionOrder(CompletionOrderSaveReqVO reqVO) {
        ...
        SceneProductionTaskEntity productionTask = requireCompletableTask(
    ...
    private SceneProductionTaskEntity requireCompletableTask(Long productionTaskId) {
        SceneProductionTaskEntity task = productionTaskRepository
                .findByIdForUpdate(productionTaskId)
                .orElseThrow(() -> new ServiceException(
                        SceneErrorCodeConstants.PRODUCTION_TASK_NOT_EXISTS));
        if (SceneTaskStatusEnum.CANCELLED.getStatus().equals(task.getTaskStatus())) {
        ...
        boolean allCompleted = !processTasks.isEmpty() && processTasks.stream().allMatch(item ->
                SceneTaskStatusEnum.COMPLETED.getStatus().equals(item.getTaskStatus()));
        if (!allCompleted) {
    ...
    private void validateReportBalance(
            SceneProductionTaskEntity productionTask, Integer completionQuantity) {
        List<SceneProcessTaskEntity> processTasks = processTaskRepository
