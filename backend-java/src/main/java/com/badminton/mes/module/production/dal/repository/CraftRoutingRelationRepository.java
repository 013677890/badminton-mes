 ...
 *
 * <p>工艺表由 A 组负责迁移和维护，本 Repository 仅按共同数据库契约读取下达工单所需事实，
 * 不引入 A 组 Entity，也不对工艺表执行写操作。
 *
 * @author 刘涵
 */
@Repository
public class CraftRoutingRelationRepository {
    ...
    private static final String ROUTING_RELATION_SQL = """
            SELECT
                ...
                EXISTS(
                    SELECT t2.id
                    FROM craft_routing_product AS t2
                    WHERE t2.routing_id = :routingId
                      AND t2.product_id = :productId
                      AND t2.is_deleted = 0
                ) AS product_bound,
                (
                    SELECT COUNT(*)
                    FROM craft_routing_detail AS t3
                    WHERE t3.routing_id = :routingId
                      AND t3.is_deleted = 0
                ) AS detail_count,
                (
                    SELECT MIN(t4.seq)
                    FROM craft_routing_detail AS t4
                    WHERE t4.routing_id = :routingId
                      AND t4.is_deleted = 0
                ) AS minimum_sequence,
                (
                    SELECT MAX(t5.seq)
                    FROM craft_routing_detail AS t5
                    WHERE t5.routing_id = :routingId
                      AND t5.is_deleted = 0
                ) AS maximum_sequence,
                (
                    SELECT COUNT(*)
                    FROM craft_routing_detail AS t6
                    LEFT JOIN craft_process AS t7
                      ON t7.id = t6.process_id
                     AND t7.status = 1
                     AND t7.is_deleted = 0
                    WHERE t6.routing_id = :routingId
                      AND t6.is_deleted = 0
                      AND t7.id IS NULL
                ) AS unavailable_process_count,
                (
                    SELECT COUNT(*)
                    FROM craft_routing_detail AS t8
                    LEFT JOIN craft_sop AS t9
                      ON t9.id = t8.sop_id
                     AND t9.sop_status = 1
                     AND (t9.effect_date IS NULL OR t9.effect_date <= CURRENT_DATE)
                     AND t9.is_deleted = 0
                    WHERE t8.routing_id = :routingId
                      AND t8.is_deleted = 0
